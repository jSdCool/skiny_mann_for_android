...
/**A solid triangular stage component that can be any color
*/
public class Sloap extends StageComponent implements Rotatable,Resizeable{//sloap component
  ...
  @param data The JSON Object containing the slope data
  */
  public Sloap(JSONObject data) {
    type="sloap";
    x=data.getFloat("x1");
    y=data.getFloat("y1");
    dx=data.getFloat("x2");
  ...
  */
  ...
  /**Create a slope  from serialized binarry data
  ...
  */
  public void draw(PGraphics render) {
    ...
    render.fill(ccolor);
    if(!isRotated()){//if not rotated
      //draw the triangle in the correct direction
      if (direction==0) {
