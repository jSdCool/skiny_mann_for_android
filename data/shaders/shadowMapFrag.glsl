...
void main(void) {
   ...
   if(lightIntensity > 0.5) {
	   ...
	   textureSection += int(!(shadowCoordProj[1].x >= 1.0 || shadowCoordProj[1].y >= 1.0 || shadowCoordProj[1].x < 0.0 || shadowCoordProj[1].y < 0.0)) * 1;
	   textureSection += int(!(shadowCoordProj[2].x >= 1.0 || shadowCoordProj[2].y >= 1.0 || shadowCoordProj[2].x < 0.0 || shadowCoordProj[2].y < 0.0)) * 2;
	   textureSection += int(!(shadowCoordProj[3].x >= 1.0 || shadowCoordProj[3].y >= 1.0 || shadowCoordProj[3].x < 0.0 || shadowCoordProj[3].y < 0.0)) * 3;
	   ...
	   if(outputSampledValue){
	     vec4 res = texture2D(shadowMap0, shadowCoordProj[textureSection].xy) * float(int(textureSection == 0));
		 res += texture2D(shadowMap1, shadowCoordProj[textureSection].xy) * float(int(textureSection == 1));
		 res += texture2D(shadowMap2, shadowCoordProj[textureSection].xy) * float(int(textureSection == 2));
		 res += texture2D(shadowMap3, shadowCoordProj[textureSection].xy) * float(int(textureSection == 3));
		 gl_FragColor = res;
		 return;
	   }
	   
	   textureSection = int(min(3.0,float(textureSection)));
	   
	   int n = 0;
	   //check if the mapped UV cord is outside of the texture without using branching 
	   n += int(shadowCoordProj[textureSection].x >= 1.0)+int(shadowCoordProj[textureSection].y >= 1.0)+int(shadowCoordProj[textureSection].x <= 0.0)+int(shadowCoordProj[textureSection].y <= 0.0)*9;
       ...
       for(; n < 9; ++n){
           visibility += step(shadowCoordProj[0].z, unpackDepth(texture2D(shadowMap0, shadowCoordProj[0].xy + poissonDisk[n] / 512.0))) * float(int(textureSection == 0))//edge dithering
		   + step(shadowCoordProj[1].z, unpackDepth(texture2D(shadowMap1, shadowCoordProj[1].xy + poissonDisk[n] / 512.0))) * float(int(textureSection == 1))
		   + step(shadowCoordProj[2].z, unpackDepth(texture2D(shadowMap2, shadowCoordProj[2].xy + poissonDisk[n] / 512.0))) * float(int(textureSection == 2))
		   + step(shadowCoordProj[3].z, unpackDepth(texture2D(shadowMap3, shadowCoordProj[3].xy + poissonDisk[n] / 512.0))) * float(int(textureSection == 3));
	   }
